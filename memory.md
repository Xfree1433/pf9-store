...
| `PLAYBOOK_LIFECYCLE_ACTIVATION.md` | Paste-ready Cowork Lifecycle agent setup. Architecture (HubSpot=state, Cowork=brain, Resend=send transport, approval queue=human gate). Trigger source is HubSpot contacts, NOT GA4 (anonymous video_play has no email). ESP decision (Resend recommended for v1). HubSpot custom-property setup. 48h dry-run protocol. Go-live L3-first. **Flags store_api.py welcome-email collision** — coordinate the agent's L3-E1 with the existing transactional welcome.|
...
- [x] **HN launch missed (6/9) — rescheduled to Tue 6/24, 8am PT.** Twitter/X thread Wed 6/25, Indie Hackers Thu 6/26. The 6/9 slot passed with no post due to founder focus on Google Ads setup. **Reminder for next session: HN is a 10–100× bigger free traffic lever than the $15/day Google Ads test — do not let ads monitoring crowd out the HN post.**

### Closed since last update (2026-06-20)
- [x] **Google Ads ghost-campaign incident** — 2026-06-20. Google's account-onboarding flow auto-created a live "Property" Search campaign at $15/day broad-match without explicit user confirmation during account setup. Drifted ~$46.84 over an extended period before being caught and removed. **Root cause:** Google's setup wizard pushes new advertisers through a "create your first campaign" flow that enables a campaign unless explicitly refused. **Mitigation:** Step 0 added to `PLAYBOOK_GOOGLE_ADS_PROPERTY_TEST.md` — refuse the onboarding flow, switch to Expert Mode, delete any auto-created campaigns before proceeding. Post-import verification step also added to catch leftovers. **Lesson generalized:** assume any new ad-platform account auto-enables spend; verify Campaigns page is empty before any setup work.
